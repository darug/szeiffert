<?php
/*******************
 * @brief ???.php fájlt állít elő a config.txt alapján,
 ...
 * 
 * @param $eltext =array(''); input mező tömb
 * @param $elsel =array('') select mező tömb
 * @param $elmul =array();    multichexbox tömb
 * @param $elrad =array();		radio gombok tömb
 * @param $d = setarrays.php-be kiírandó tartalmat gyűjti
 * @param $t = index.php elejére kiírandó tartalmat gyűjti
 * @param $t0 = fnev.php fajl eleje
 * @param $st = fnev.php végére kiírandó tartalmat gyűjti
 * @param $m = smcreate.php-be kiírandó tartalmat gyűjti
 * @param $fontos array('f0'=>'&nbsp','f1'=>'1=egyáltalán nem fontos','f2'=>'2=nem fontos','f3'=>'3=normál','f4'=>'4=fontos','f5'=>'5=igen fontos') 
 * @param $gyakor=array('g0'=>'0=soha','g1'=>'1=hetente','g2'=>"2=havonta",'g3'=>'3=negyedévente','g4'=>'4=félévente','g5'=>'5=évente');
 * @param $sstat= változóba gyüjtjük össze a fnevstat.php-ba írandó programrészt
 * 
 * @author iDG
 *
 * @todo: 2012.06.30. atalakitani, hogy: 
 * 2015.06.07. mysql függvények kiváltása és a statisztikai függvények bővítése átalakítás
 * 
 **************************/
 $f=explode("/",$_SERVER["REQUEST_URI"]);
 if($f[1]==dem){ $rt='/dem/'; } //notebookon futtatáshoz
  else{ $rt=''; }
...
if(include("mysql/mysqli_connect.php")){echo "Sikeres mysqli_connect inlude a smcreate fájlban!<br>";}else{echo "Sikertelen mysqli_connect inlude a smcreate fájlban!<br>";}
$msq="drop table IF EXISTS $db_tablanev_table";
if(mysqli_query($mysqli,$msq)){print ("<B>Sikeres törlés</B><br>");} else {print("Adatbázis törlési hiba!!!".mysqli_error($mysqli))."<br>";}
...
//            unlink('database.php');
  ...
  //      }
        ...
        case 'text':
			...
			$ststat.='<?php'."\n";
			$ststat.="\$i=$ii;\n";
            $ststat.="echo Inputstat(\$eltext[\$i]['name'],\$eltext[\$i]['label'],\$eltext[\$i]['class'],\$db_tablanev_table,\$mysqli);\n" ;
            $ststat.='?>'."\n<br>";
            break;
        case 'select':
            $selnumend++; 
			...
			$ststat.='<?php'."\n";
            $ststat.="\$i=$ii;\n";
            $ststat.="echo Radiostat(\$elsel[\$i]['name'],\$elsel[\$i]['label'],\$elsel[\$i]['list'],\$db_tablanev_table,\$mysqli);\n" ;
            $ststat.='?>'."\n<br>";
            break;
        case 'multicheckbox':
            $mulnumend++;
            ...
            $st.='<?php'."\n";
            $st.="\$i=$ii;\n";
            $st.="echo multicheckbox(\$elmul[\$i]['name'],\$elmul[\$i]['label'],\$elmul[\$i]['list']);\n" ;
            $st.='?>'."\n<br>";
			//$ststat még ellenőrzésre vár!!!
			$ststat.='<?php'."\n";
            $ststat.="\$i=$ii;\n";
            $ststat.="echo Multicheckstat(\$elmul[\$i]['name'],\$elmul[\$i]['label'],\$elmul[\$i]['list'],\$db_tablanev_table,\$mysqli);\n" ;
            $ststat.='?>'."\n<br>";
            break;
        case 'combo':
            $combonumend++;
            ...
            $st.='<?php'."\n";
            $st.="\$i=$ii;\n";
            $st.="echo combo(\$elcom[\$i]['name'],\$elcom[\$i]['label1'],\$elcom[\$i]['list1'],\$elcom[\$i]['kerdes'],\$elcom[\$i]['label2'],\$elcom[\$i]['list2']);\n" ;
            $st.='?>'."\n<br>";
		...
		case 'radio':
            ...
            $st.='<?php'."\n";
            $st.="\$i=$ii;\n";
            $st.="echo radio(\$elrad[\$i]['name'],\$elrad[\$i]['label'],\$elrad[\$i]['list']);\n" ;
            $st.='?>'."\n<br>";
            $ststat.='<?php'."\n";
            $ststat.="\$i=$ii;\n";
            $ststat.="Radiostat(\$elrad[\$i]['name'],\$elrad[\$i]['label'],\$elrad[\$i]['list'],\$db_tablanev_table,\$mysqli);\n" ;
            $ststat.='?>'."\n<br>";
            break;
        default:     
...
if (!$fd) {
    echo "Could not open file setarrays$fnv.php";
}else {
print('249. sor: setarrays'.$fnv.'.php ok: '.$fd.'<br>');
}
...
$t0.="include ('form/form_stat1.php');\n";
$t0.="include ('mysql/mysqli_connect.php');\n";
$t0.="include ('mysql/mysqli_lekerdez.php'); //l() függvény, gyakorlatilag a mysql_query() rövidítése\n";
$t0.= "?>\r\n";
// yii változatban nem kell header,ezért megjegyzésbe rakható
...
$select="SELECT * from `dtk` ;";
	if($lis=mysqli_query($select)){
		$sum=mysqli_num_rows($lis);}
?>';
...
$st0.='<form action="'.$fnev.'" method="post">'."//yíí-ben átnevezendő!!\n";
$st0.='<input type="hidden" name="form" id="form" value="true" />'."\n";
$t=$t0.$t;
$tstat=$t0stat.$tstat;
$st=$st0.$st;
$ststat=$st0stat.$ststat;
if(include_once("database".$fnv.".php")){print "320 sorban sikeres adabázis includolás!<br>";};
$fw=fopen($fnev,'w');
...
?> <form action="'.$fnevstat.'" method="post"><!-   yii-ben megváltoztandó!!! -->
...
print($msq."<br>");
if(mysqli_query($mysqli,$msq)){print ("<B>Sikeres létrehozás</B><br>");
$ok=true;} else {print("<font color=\"red\">Adatbázis létrehozási hiba!!!".mysqli_error($mysqli)."</font>");
$ok=false;}

?>';
$st.='include_once("mysql/mysqli_connect.php");
...
?>';
print($st);$i=569;
if(file_put_contents($fnev, $st,FILE_APPEND)==false ){print($fnev.' fajlba iras nem sikerult 560. sorban<br>');} else {print"$i. sor sikeres $fnev fajlbairas<br>";}
if(file_put_contents($fnevstat, $ststat,FILE_APPEND)==false ){print($fnevstat.' fajlba iras nem sikerult<br>');} else {print"$i+1. sor sikeres $fnevstat fajlbairas<br>";}
...
if(include_once("database".$fnv.".php")){print "577. sorban sikeres adabázis includolás!<br>";}
//print $db_user."<br>";
//print $db_passwd;
if(include("smcreate.php")){echo "580.sorban sikeres smcreate.php include<br>";} else {echo "580.sorban sikertelen smcreate.php include<br>";}
echo $db_tablanev_table." a tábla neve<br>";
