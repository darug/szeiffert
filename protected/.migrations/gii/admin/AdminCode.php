...
class AdminCode extends CCodeModel
{
	...
	public function requiredTemplates()
	{
		return array(
			...
			'controllers'.DIRECTORY_SEPARATOR.'admin'.DIRECTORY_SEPARATOR.'controller.php',
		);
	}
	...
	public function prepare()
	{
		...
		/*
		 * 
		 * FLAT DIRECTORY SCAN AND FILE GENERATING
		 * 
		 */
		...
		/*
		 * 
		 * RECURSIVE DIRECTORY SCAN AND FILE GENERATION TO THE SAME PATH
		 * 
		 */
		...
		foreach($files as $file)
		{
				...
				$templatePath;
				$relativeFilePath = substr($filePath,strlen($templatePath));
				
				if($fileName == "controller.php"){
					
					$replaceTo = $this->getControllerID().'Controller.php';
					$relativeFilePath = $relativeFilePath.$replaceTo;

				}
				if(preg_match('/views/', $relativeFilePath)){
					
					$relativeFilePath = $relativeFilePath.$this->getControllerID().DIRECTORY_SEPARATOR.$fileName;

				}
				
				$generatePath = Yii::getPathOfAlias('application').$relativeFilePath;
